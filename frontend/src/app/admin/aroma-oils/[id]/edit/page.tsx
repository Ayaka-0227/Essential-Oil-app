...
export default function AdminAromaOilEditPage() {
  ...
  useEffect(() => {
    ...
    fetch(apiUrl("/admin/aroma_oils"), {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then((r) => r.json())
      .then((oils: { id: number; name: string; description: string; category: string; scent_category: string }[]) => {
        const oil = oils.find((o) => String(o.id) === id);
        if (oil) {
          setName(oil.name);
          setDescription(oil.description);
          setSelectedMentalCategories(oil.category ? oil.category.split(",").map((s) => s.trim()).filter(Boolean) : []);
          setScentCategory(oil.scent_category ?? "");
        }
        else setError("オイルが見つかりませんでした");
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(apiUrl(`/admin/aroma_oils/${id}`), {
        method: "PUT",
        headers: {
      ...
      if (!res.ok) {
        ...
        throw new Error(data.errors?.join(", ") ?? "更新に失敗しました");
      }
  ...
  return (
    <main className="min-h-screen bg-[#f7f4ef] px-6 py-10 text-stone-800">
      ...
      <div className="mx-auto w-full max-w-lg">
        <h1 className="mb-6 text-2xl font-semibold tracking-tight">アロマオイル更新</h1>
